				   ...
				   <?php
						...
						if(mysqli_affected_rows($conn)>0)
						{
							while($row=mysqli_fetch_assoc($result))
							{
								echo $row['replier'];?>  :
								<?php 
	echo $row['reply'];?><a href="admin_q_reply_backend.php?id=<?php echo $_GET['id'];?>&r_id=<?php echo $row['r_id'];?>">&nbsp;&nbsp;&nbsp;&nbsp;<span style="color:#FF0000" class="fui-cross"></span></a><br /><small>Replied on :
									<?php 
									$time=$row['time'];
									$phpdate = strtotime( $time );
									$time = date( 'd-m-Y H:i:s', $phpdate );
